...
Dictionary<byte, Xbox360Property> analogMap = new Dictionary<byte, Xbox360Property>{
    ...
    {17, Xbox360Slider.LeftTrigger},
    {18, Xbox360Slider.RightTrigger}
};
...
server.Start(socket =>{
    ...
    socket.OnMessage = message =>{
        ...
        if(resArray.RootElement.ValueKind == JsonValueKind.Array)
            ...
            (byte)resArray.RootElement[1].GetInt16());
    };
    ...
    socket.OnBinary = message=>{
        ...
        if(message[1] < 15) controller.SetButtonState(message[1], message[2] == 255);
        else if(message[1] == 17 || message[1] == 18) controller.SetSliderValue((Xbox360Slider)analogMap[message[1]], message[2]);
    };
});
